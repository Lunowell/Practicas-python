...
def layout_seleccionar_template(templates):
    ...
    layout = [[sg.Text(
        ...
        highlight_text_color='white'),
            sg.Image(
        data=abrir_foto_collage(os.path.join(os.getcwd(),"Fotos","Fondo_Meme.png")),
        key='-IMAGE-')],
            [columna]]
...
def window_seleccionar_template(alias):
    ...
    templates = cargar_lista_json("coordenadas.json")
        ...
        if event == "-GENERAR-":
            if (values["-ARCHIVOS-"] != []):
                ...
                for arc in templates:
                    if (arc['name'] == values["-ARCHIVOS-"][0]):
                        datos = arc
                collage_seleccionado = os.path.join(os.getcwd(),"Fotos",datos["image"])
                textbox = (datos["text_boxes"])
                generar_collage.window_generar_collage(alias,collage_seleccionado,textbox)
                window.UnHide()
    window.close()